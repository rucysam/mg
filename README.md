...
```
index.html                   화면 구조 (타이틀 / 게임 / 결과 / 게임 방법 4개 화면, 난이도 선택 UI 포함)
css/
  style.css                   전체 디자인(색상·레이아웃·12개 미니게임 스타일·타이머/콤보/게임 방법 UI)
js/
  icons.js                    인라인 SVG 아이콘 모음 (window.GameIcons)
  sound.js                    전자음 사운드 이펙트 시스템 (window.GameSound, 음원 파일 없음)
  howto.js                    "게임 방법" 화면에 쓰이는 12개 미니게임 미리보기+설명 데이터 (window.HOWTO_ENTRIES)
...
타이틀 화면의 "게임 방법" 카드(`#btn-howto`)를 누르면 12개 미니게임을 하나씩 미리 보여주는 별도 화면
(`#screen-howto`)으로 이동합니다. 각 항목은 실제 게임 화면과 똑같은 CSS 클래스를 재사용해 만든 "정지된
미리보기"(탭해도 아무 동작도 하지 않음, `pointer-events: none`) 위에 제목과 설명을 붙인 형태입니다.

이 내용은 `js/howto.js`의 `window.HOWTO_ENTRIES` 배열에서 관리하며, 각 항목은 다음 구조입니다.

```js
{
  title: '표지판 인식',
  description: '플레이 방법을 설명하는 한두 문장',
  renderPreview: function (container) {
    // 실제 스테이지와 같은 CSS 클래스를 재사용해 정적인 미리보기 DOM을 만들어 넣습니다.
  }
}
```

**주의**: 이 파일은 실제 스테이지 파일(`js/stages/*.js`)과 완전히 분리되어 있어서, 게임 규칙(허용 오차,
개수, 시간 등)을 바꿔도 이 파일은 자동으로 갱신되지 않습니다. 단계의 플레이 방법이 바뀌면 `js/howto.js`의
해당 항목 `description`도 함께 손으로 고쳐줘야 합니다.
...
| `stage-sort.js` | 막대 개수(`count`), 제한시간, 점수 페널티(`PENALTY_PER_EXTRA_SWAP`), 최소 보장 점수(`MIN_SCORE`) |
...
| `sound.js`의 `BGM_CHORDS` / `BGM_ARP_NOTES` / `BGM_CHORD_DURATION` | 배경음의 코드 진행·아르페지오 음·코드 길이(템포) |

## 사운드 시스템

`js/sound.js`는 오디오 파일을 전혀 쓰지 않고, Web Audio API의 오실레이터로 모든 사운드를 즉석에서 합성합니다
(아이콘을 이미지 대신 SVG로 그리는 것과 같은 접근 방식입니다). 효과음과 배경음(BGM) 두 가지로 구성됩니다.

**효과음**: `window.GameSound.play('success')`처럼 이름으로 호출하면 되고, 현재 다음 상황에 자동으로 연결되어 있습니다.
...
**배경음(BGM)**: `window.GameSound.startMusic()`을 호출하면 차분한 마이너 코드 진행(Am-F-G-Em, 코드당 3.2초)이
무한히 이어집니다. 반복 청취해도 지겹지 않도록 (1) 효과음보다 훨씬 낮은 볼륨으로 배경에 깔리고, (2) 짧은 루프가
아니라 4개 코드가 도는 진행을 쓰고, (3) 코드 위에 얹는 아르페지오 음을 매번 무작위로 골라 매 루프가 완전히
똑같이 반복되지 않게 했습니다. "인증 시작하기"를 누르면 시작되고, 한 번 시작되면(`재도전`을 눌러도) 끊김 없이
이어집니다. 음 높이·코드·볼륨은 `sound.js`의 `BGM_CHORDS`/`BGM_ARP_NOTES`/`BGM_CHORD_DURATION`에서 조정할 수 있습니다.

화면 우상단의 음소거 버튼(`#btn-mute`)으로 효과음과 배경음을 한 번에 켜고 끌 수 있고, 그 상태는 `localStorage`에
저장되어 다음 방문 때도 유지됩니다. 모바일 브라우저의 자동재생 정책 때문에, 반드시 사용자의 클릭(시작 버튼,
음소거 버튼, 난이도 버튼 등) 안에서 `window.GameSound.unlock()`을 한 번 호출해 오디오 컨텍스트를 깨워둡니다.

## 알려진 수정 사항

- **크기 순서 맞추기(`stage-sort.js`) 채점 기준 개선 (2차)**: 처음에는 막대를 교체할 때마다 몰래 정렬 여부를
  검사해서 "자동으로" 통과시키는 방식이었는데, 사용자가 의도하지 않은 순간에 갑자기 통과/실패하고 통과
  기준도 화면에 보이지 않아 혼란스럽다는 피드백을 받아 "확인" 버튼을 직접 누르는 방식으로 바꿨습니다.
  하지만 그렇게 하면 사용자가 이미 올바르게 정렬을 끝냈어도 확인을 누르기 전까지는 끝나지 않는, "끝나는
  시점이 불분명한" 상태가 생기고 그 사이 제한시간이 끝나버리면 다 맞췄는데도 실패한 것처럼 보이는 문제가
  있었습니다. 그래서 최종적으로는 확인 버튼을 없애고, 막대 순서가 규칙대로 맞는 그 즉시 자동으로 종료되도록
  바꿨습니다. 점수도 고정 100점이 아니라 "최소 교체 횟수(순열의 순환 분해로 계산) 대비 실제로 사용한 교체
  횟수"를 기준으로 누적/차감되도록(추가 교체 1번당 -10점, 최소 50점 보장) 바꿔서, 막대(사이즈)를 효율적으로
  맞출수록 점수가 쌓이는 구조를 명확히 했습니다. 최적 교체로 풀면 100점, 비효율적으로 풀어도 통과만 하면
  최소 50점을 받습니다. 이 동작은 main.js 없이 스테이지만 독립적으로 호출하는 단위 테스트로
  (최적 풀이=100점, 비효율적 풀이=점수 차감, 매우 비효율적 풀이=50점 바닥, 같은 막대 재탭=선택취소만 되고
  교체 횟수에 영향 없음) 모두 확인했습니다.
- **스와이프/길게 누르기 단계 포인터 캡처**: 손가락이 입력 영역 밖으로 나가도 떼는 동작이 정상 인식되도록
  `setPointerCapture`를 추가했습니다.
- **실제 모바일 기기에서 표지판 인식(1단계) 타일이 화면 밖으로 잘려 보이는 문제**: 아이콘 SVG에 `width`/`height`
  속성이 없으면 일부 브라우저(특히 WebKit 계열, 예: iOS의 인앱 브라우저)는 SVG의 고유 크기를 기본값인
  300×150으로 취급해서 CSS Grid의 칸 너비 계산에 그 값을 그대로 반영합니다. 화면에는 작게 그려지더라도
  그리드 칸 자체가 300px 이상으로 넓어져 버리는 것이 원인이었습니다(Chromium 기반 테스트 환경에서는 이
  문제가 재현되지 않아 자동 테스트만으로는 발견하지 못했고, 실제 기기 스크린샷으로 발견했습니다).
  모든 아이콘 SVG에 `viewBox`와 일치하는 `width`/`height` 속성을 명시적으로 넣어 근본 원인을 없앴고,
  같은 종류의 문제를 막기 위해 그리드/플렉스 아이템에 `min-width: 0`을 광범위하게 추가했습니다.
  이와 함께 음소거 버튼이 게임 화면 헤더의 목숨 아이콘과 겹치는 문제도 발견해, 음소거 버튼을 `#app` 카드
  기준 좌표로 옮기고 헤더에 여유 공간을 확보했으며, 단계 진행 점(`.dot`) 12개가 좁은 화면에서 자기 콘텐츠
  크기 때문에 트랙을 강제로 넓히던 부분도 `min-width:0` + 가변 너비로 고쳤습니다. 320~430px 폭 구간에서
  가로 스크롤·겹침이 없는지 Playwright로 다시 확인했습니다. (단, 이 자동 테스트는 Chromium 기반이라 원래
  문제가 발생했던 WebKit 계열 브라우저에서의 최종 확인은 실제 기기에서 한 번 더 봐주시는 게 좋습니다.)
...
- `stage-sort.js`는 main.js 없이 독립적으로 호출해(`stage.init(container, mockCtx)`) 최적 교체로 풀면 100점,
  비효율적으로 풀면 점수가 깎이고 매우 비효율적으로 풀어도 최소 50점 바닥에서 통과하는지, 같은 막대 재탭은
  교체 횟수에 영향 없이 선택만 취소되는지를 단위 테스트로 확인했습니다.
- 배경음(BGM)을 10초 이상(코드 진행 3바퀴 이상) 재생해도 콘솔 에러가 없는지, 음소거 토글이 효과음/배경음
  모두에 정상적으로 적용되는지 확인했습니다.
- 390×844(모바일) 화면 크기에서 타이틀(난이도 선택 포함)·12개 단계 전체·결과 화면을 스크린샷으로 검토했습니다.
- **게임 방법 화면**: 타이틀의 "게임 방법" 카드를 누르면 12개 항목이 모두(빈 내용 없이) 렌더링되는지, 뒤로가기로
  타이틀로 돌아온 뒤에도 "인증 시작하기"가 정상적으로 게임을 시작하는지(화면 전환 상태가 꼬이지 않는지)
  확인했습니다. 320px(좁은 화면)에서도 가로 스크롤 없이 미리보기들이 깨지지 않는지 함께 확인했습니다.
