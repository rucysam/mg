...
- **크기 순서 맞추기(`stage-sort.js`) 채점 기준 개선**: 예전에는 막대를 교체할 때마다 몰래 정렬 여부를 검사해서
  "자동으로" 통과시키는 방식이었는데, 사용자가 의도하지 않은 순간에 갑자기 통과/실패하고 막대 높이만으로는
  통과 기준을 가늠하기 어렵다는 문제가 있었습니다. 지금은 막대마다 숫자를 표시하고, 다른 단계들처럼 "확인"
  버튼을 직접 눌렀을 때만 채점하도록 바꿔서 기준을 명확하게 했습니다. (단위 테스트로 비정렬 상태에서 확인 시
  정상적으로 실패하는지, 정렬 완료 후 확인 시 정상적으로 통과하는지, 같은 막대를 두 번 탭했을 때 선택이
  올바르게 취소되는지를 모두 확인했습니다.)
...
- `stage-sort.js`는 main.js 없이 독립적으로 호출해(`stage.init(container, mockCtx)`) 비정렬 상태에서 확인 시
  실패, 정렬 완료 후 확인 시 통과, 같은 막대 재탭 시 선택취소가 모두 올바른지 단위 테스트로 확인했습니다.
